...
if(jQuery)( function() {
	...
	$.extend($.fn, {
		...
		contextMenu: function(o, callback, contextChange) {
			// Defaults
				...
				$(this).bind( "mousedown", function(e) {
					...
					$(this).mouseup( function(e) {
						...
						if( evt.button === 2 || o.allowLeft || (evt.ctrlKey && svgedit.browser.isMac()) ) {
                            ...
                            // call contextChange function that enables/disables the right stuff
                            if(contextChange){ contextChange(e); }

							var x_off = win.width() - menu.width(), 
								y_off = win.height() - menu.height();

                            //alert("(x " + x + ")(y " + y + ")(x_off " + x_off + ")(y_off " + y + ")");
							...
							menu.find('.contextMenuLi:not(.disabled) > a').mouseup( function() {
		...
		disableContextMenuItems: function(o) {
			if( o == undefined ) {
				// Disable all
				$(this).find('LI').addClass('disabled').hide();
				return( $(this) );
			}
			$(this).each( function() {
				if( o != undefined ) {
					...
					for( var i = 0; i < d.length; i++ ) {
						$(this).find('a[href="' + d[i] + '"]').parent().addClass('disabled').hide();
					}
				}
			});
			return( $(this) );
		...
		enableContextMenuItems: function(o) {
			if( o == undefined ) {
				// Enable all
				$(this).find('LI.disabled').removeClass('disabled').show();
				return( $(this) );
			}
			$(this).each( function() {
				if( o != undefined ) {
					var d = o.split(',');
					for( var i = 0; i < d.length; i++ ) {
						$(this).find('a[href="' + d[i] + '"]').parent().removeClass('disabled').show();
